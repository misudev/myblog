...
const Wrapper = ({
  ...
  return /*#__PURE__*/_react.default.createElement(_errorBoundary.default, null, /*#__PURE__*/_react.default.createElement(GlobalsProvider, {
    value: {
...
const render = (recipe, cb, inputs = {}, isApply, isStream, name) => {
  ...
  const renderResources = isDrained => {
    result = _reconciler.default.render(recipeWithWrapper, plan, name);
    const resources = (0, _transformToPlanStructure.default)(result);
    ...
    const isDone = () => {
      ...
      if (resources.length === 0 && ![...inFlightCache.values()].some(a => a)) {
        return true; // If there's still nothing on the queue and we've drained the queue, that means we're done.
      } else if (isDrained && queue.length === 0) {
        return true; // If there's one resource & it fails validation, it doesn't go into the queue
        // so we check if inFlightCache is empty & all resources have an error.
      } else if (!resources.some(r => !r.error) && ![...inFlightCache.values()].some(a => a)) {
        return true;
      }
  ...
  const throttledRenderResources = _lodash.default.throttle(renderResources, 30, {
    trailing: false
  });
