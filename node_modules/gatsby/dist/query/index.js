...
const getInitialQueryProcessors = ({
  ...
  const {
    staticQueryIds,
    pageQueryIds
  } = groupQueryIds(queryIds);
  const queryjobsCount = _.filter(pageQueryIds.map(id => state.pages.get(id))).length + staticQueryIds.length;
  let activity = null;
  ...
  const createProcessor = (fn, queryIds) => async () => {
    ...
    processedQueuesCount++; // if both page and static queries are done, finish activity
