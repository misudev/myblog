...
const FAST_OPS = [`$eq`, `$ne`, `$lt`, `$lte`, `$gt`, `$gte`]; // More of a testing mechanic, to verify whether last runSift call used Sift
...
const createFilterCacheKey = (typeNames, filter) => {
  ...
  while (f) {
    ...
    if (f.type === `elemMatch`) {
      ...
      /*: IDbQueryElemMatch*/
      = f;
      f = q.nestedQuery; // Make distinction between filtering `a.elemMatch.b.eq` and `a.b.eq`
...
function handleFirst(siftArgs, nodes) {
  ...
  const index = _lodash.default.isEmpty(siftArgs) ? 0 : nodes.findIndex(sift({
    $and: siftArgs
  }));
...
function handleMany(siftArgs, nodes) {
  let result = _lodash.default.isEmpty(siftArgs) ? nodes : nodes.filter(sift({
    $and: siftArgs
  }));
...
const runFiltersWithoutSift = (filters, nodeTypeNames, filtersCache) => {
  const nodesPerValueSets
  /*: Array<Set<IGatsbyNode>> */
  = getBucketsForFilters(filters, nodeTypeNames, filtersCache);
  ...
  nodesPerValueSets.sort((a
  /*: Set<IGatsbyNode> */
  , b
  /*: Set<IGatsbyNode> */
  ) => b.size - a.size); // Iterate on the set with the fewest elements and create the intersection

  const needles
  /*: Set<IGatsbyNode>*/
  = nodesPerValueSets.pop(); // Take the intersection of the retrieved caches-by-value
  ...
  /*: IGatsbyNode */
    ...
    if (nodesPerValueSets.every((cache
    /*: Set<IGatsbyNode> */
    ) => cache.has(node))) {
...
const getBucketsForFilters = (filters, nodeTypeNames, filtersCache) => {
  ...
  let every = filters.every((filter
  /*: DbQuery*/
  ) => {
    ...
    if (filter.type === `query`) {
      ...
      /*: IDbQueryQuery */
      = filter;
      return getBucketsForQueryFilter(filterCacheKey, q, nodeTypeNames, filtersCache, nodesPerValueSets);
    } else {
      // (Let TS warn us if a new query type gets added)
      const q
      /*: IDbQueryElemMatch*/
      = filter;
      return collectBucketForElemMatch(filterCacheKey, q, nodeTypeNames, filtersCache, nodesPerValueSets);
    }
  });
...
const getBucketsForQueryFilter = (filterCacheKey, filter, nodeTypeNames, filtersCache, nodesPerValueSets) => {
  let {
    path: filterPath,
    query: {
  ...
  if (!filtersCache.has(filterCacheKey)) {
    ensureIndexByQuery(comparator, filterCacheKey, filterPath, nodeTypeNames, filtersCache);
  }

  const nodesPerValue
  /*: Set<IGatsbyNode> | undefined */
  = getNodesFromCacheByValue(filterCacheKey, filterValue, filtersCache); // If we couldn't find the needle then maybe sift can, for example if the
...
const collectBucketForElemMatch = (filterCacheKey, filter, nodeTypeNames, filtersCache, nodesPerValueSets) => {
  ...
  if (!filtersCache.has(filterCacheKey)) {
    ensureIndexByElemMatch(comparator, filterCacheKey, filter, nodeTypeNames, filtersCache);
  }

  const nodesByValue
  /*: Set<IGatsbyNode> | undefined*/
  = getNodesFromCacheByValue(filterCacheKey, targetValue, filtersCache); // If we couldn't find the needle then maybe sift can, for example if the
  // schema contained a proxy; `slug: String @proxy(from: "slugInternal")`
  // There are also cases (and tests) where id exists with a different type
...
const runFilterAndSort = args => {
  ...
  const result = applyFilters(filter, firstOnly, nodeTypeNames, filtersCache, resolvedFields, stats);
...
const applyFilters = (filterFields, firstOnly, nodeTypeNames, filtersCache, resolvedFields, stats) => {
  const filters
  /*: Array<DbQuery>*/
  = filterFields ? prefixResolvedFields(createDbQueriesFromObject(prepareQueryArgs(filterFields)), resolvedFields) : [];
  ...
  const siftResult = filterWithSift(filters, firstOnly, nodeTypeNames, resolvedFields);
...
const filterToStats = (filter
/*: DbQuery*/
, filterPath = [], comparatorPath = []) => {
...
const filterWithoutSift = (filters, nodeTypeNames, filtersCache) => {
  ...
  if (filters.some(filter => filter.type === `query` && !FAST_OPS.includes(filter.query.comparator))) {
...
const filterWithSift = (filters, firstOnly, nodeTypeNames, resolvedFields) => {
  ...
  return runSiftOnNodes(nodes, filters.map(f => dbQueryToSiftQuery(f)), firstOnly, nodeTypeNames, resolvedFields, siftGetNode);
};
/**
...
const runSiftOnNodes = (nodes, filters, firstOnly, nodeTypeNames, resolvedFields, getNode) => {
...
const sortNodes = (nodes, sort, resolvedFields, stats) => {
  if (!sort || (nodes === null || nodes === void 0 ? void 0 : nodes.length) <= 1) {
    return nodes;
