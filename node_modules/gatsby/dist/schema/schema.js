  ...
  schemaComposer.forEach(typeComposer => {
    if (typeComposer instanceof ObjectTypeComposer || typeComposer instanceof InterfaceTypeComposer) {
      typeComposer.getFieldNames().forEach(fieldName => {
        const field = typeComposer.getField(fieldName);
        typeComposer.extendField(fieldName, {
          resolve: field.resolve ? tracingResolver(field.resolve) : defaultTracingResolver
        });
      });
    }
  });
...
const groupChildNodesByType = ({
  nodeStore,
  nodes
}) => _(nodes).flatMap(node => (node.children || []).map(nodeStore.getNode).filter(Boolean)).groupBy(node => node.internal ? node.internal.type : undefined).value();
